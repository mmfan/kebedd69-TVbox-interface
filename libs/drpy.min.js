...
const OCR_RETRY = 3;
const OCR_API = "http://cms.nokia.press/parse/ocr";
...
function dealJson(html) {
    try {
        return html.match(/[\w|\W|\s|\S]*?(\{[\w|\W|\s|\S]*\})/)
            .group[1]
    } catch (e) {}
    return html
}
    ...
    if (p === "*") {
        ...
        if (p.lists) {
            for (let i = 0; i < playFrom.length; i++) {
                ...
                console.log(tabName);
                vodList.forEach(it => {
                    new_vod_list.push(cheerio.load(it)
                        .text() + "$" + _pd(it, "a&&href", MY_URL))
                });
